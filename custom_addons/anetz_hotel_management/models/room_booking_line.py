...
class RoomBookingLine(models.Model):
    ...
    tax_ids = fields.Many2many('account.tax', 'hotel_room_order_line_taxes_rel', 'room_id', 'tax_id',
        related='room_id.taxes_ids', string='Taxes', help="Default taxes used when selling the room." , domain=[('type_tax_use', '=', 'sale')])
                    ...
                    if line.id != rec.id and line.room_id == rec_room_id:
                        ...
                        if (rec_checkin_date <= line.checkin_date <= rec_checkout_date or
                            ...
                            raise ValidationError(
                                _("Sorry, You cannot create a reservation for this date since it overlaps with another reservation..!!"))
                        if rec_checkout_date <= line.checkout_date and rec_checkin_date >= line.checkin_date:
                            raise ValidationError(
                                "Sorry You cannot create a reservation for this ate due to an existing reservation between this date")
