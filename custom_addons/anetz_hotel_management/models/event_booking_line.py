...
class EventBookingLine(models.Model):
    ...
    ticket_id = fields.Many2one('product.product', string="Ticket", help="Choose the Ticket Type", domain=[('detailed_type', '=', 'event')])
    ...
    tax_ids = fields.Many2many('account.tax', 'hotel_event_order_line_taxes_rel', 'event_id', 'tax_id', related='ticket_id.taxes_id',
        string='Taxes', help="Default taxes used when selling the event tickets.", domain=[('type_tax_use', '=', 'sale')])
