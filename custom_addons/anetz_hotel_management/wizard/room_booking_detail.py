...
class RoomBookingWizard(models.TransientModel):
    ...
    def action_room_booking_pdf(self):
        ...
        return self.env.ref(
            "anetz_hotel_management.action_report_room_booking"
    ...
    def generate_data(self):
        ...
        room_booking = self.env["room.booking"].search_read(domain=domain, fields=["partner_id", "name", "checkin_date", "checkout_date"],)
        for rec in room_booking:
            rooms = (
                self.env["room.booking"].browse(rec["id"]).room_line_ids.room_id.mapped("name")
            )
