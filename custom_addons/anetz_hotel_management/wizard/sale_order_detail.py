...
class SaleOrderWizard(models.TransientModel):
    ...
    def action_sale_order_pdf(self):
        ...
        return self.env.ref('anetz_hotel_management.action_report_sale_order').report_action(self, data=data)
    ...
    def generate_data(self):
        ...
        room_booking = self.env['room.booking'].search_read(domain=domain, fields=[
            'partner_id',
            'name',
            'checkin_date',
            'checkout_date',
    ...
    def get_xlsx_report(self, data, response):
        ...
        for i in data['booking']:
            ...
            sheet.write(row, column + 5, "{:.2f}".format(i['amount_total']), body)
            row = row + 1
            value = value + 1
        workbook.close()
        output.seek(0)
